...
export interface RateLimitGaugeProps {
  ...
  warningThreshold?: number; // Percentage at which to show warning color (default: 70)
  criticalThreshold?: number; // Percentage at which to show critical color (default: 90)
}
 ...
 */
export function renderBasicDashboardHTML(props: ApiMonitorDashboardProps): string {
          ...
          <select id="plan-select">
            ${Object.keys(planConfig.availablePlans).map(plan => `
              <option value="${plan}" ${plan === planConfig.currentPlan ? 'selected' : ''}>
                ${plan.charAt(0).toUpperCase() + plan.slice(1)} (${planConfig.availablePlans[plan as ShopifyPlan].pointsPerSecond} points/sec)
              </option>
            `).join('')}
          </select>
